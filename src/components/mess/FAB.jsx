...
export default function FAB({ messId, members, myRole, userId, inviteCode, messName, currentMonth }) {
  ...
  // Only manager sees all options; members only see Add Meal (for themselves)
  ...
  const mealMembers = isManager ? members : members.filter(m => m.userId === userId);
    ...
    <>
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map(cat => (
                  <button key={cat.label} onClick={() => setExpCat(cat.label)}
                    className="chip font-semibold text-xs px-3 py-1.5 rounded-xl"
      ...
      {isManager && (
        <BottomSheet open={sheet === 'invite'} onClose={closeSheet} title="Invite Member">
          <div className="px-5 pb-6">
            <p className="text-sm text-gray-500 mb-4">Share this code to invite someone to <strong>{messName}</strong>.</p>
