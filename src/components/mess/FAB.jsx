    ...
    <>
      <div className="fixed bottom-6 right-4 z-40 flex flex-col items-end gap-3">
        {open && fabItems.map((item, i) => {
          ...
          return (
            <div key={item.key} className="fab-item flex items-center gap-2" style={{ animationDelay: `${i * 0.04}s` }}>
              <span className="bg-gray-900 text-white text-xs font-semibold px-3 py-1.5 rounded-xl shadow-lg">{item.label}</span>
              <button onClick={() => openSheet(item.key)}
        ...
        <button onClick={() => setOpen(p => !p)}
          className="w-14 h-14 rounded-full text-white flex items-center justify-center shadow-fab active:scale-90 transition-transform"
      ...
      <BottomSheet open={sheet === 'meal'} onClose={closeSheet} title="Add Meals">
        <div className="px-5 pb-6">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['breakfast','lunch','dinner'].map(meal => (
                    <div key={meal}>
                      ...
                      <input type="number" min="0" max="3" step="0.5" className="inp text-center text-sm py-2" placeholder="0"
          ...
          <button onClick={saveMeals} disabled={saving}
            className="w-full mt-4 py-3 rounded-2xl font-bold text-white" style={{ background: '#E60023' }}>
      ...
      <BottomSheet open={sheet === 'expense'} onClose={closeSheet} title="Add Expense">
        <div className="px-5 pb-6 space-y-3">
          ...
          <input type="number" className="inp" placeholder="Amount (৳)" value={expAmount} onChange={e => setExpAmount(e.target.value)} min="0" />
          <div>
            <p className="text-xs font-bold text-gray-500 mb-2">Category</p>
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map(cat => (
                <button key={cat.label} onClick={() => setExpCat(cat.label)} className="chip font-semibold text-xs"
          ...
          <input className="inp" placeholder="Description (optional)" value={expDesc} onChange={e => setExpDesc(e.target.value)} />
          <input type="date" className="inp" value={expDate} onChange={e => setExpDate(e.target.value)} max={today} />
      ...
      <BottomSheet open={sheet === 'payment'} onClose={closeSheet} title="Add Payment">
        <div className="px-5 pb-6 space-y-3">
          ...
          <input type="number" className="inp" placeholder="Amount (৳)" value={payAmount} onChange={e => setPayAmount(e.target.value)} min="0" />
          <div>
          ...
          <input className="inp" placeholder="Note (optional)" value={payNote} onChange={e => setPayNote(e.target.value)} />
          <input type="date" className="inp" value={payDate} onChange={e => setPayDate(e.target.value)} max={today} />
